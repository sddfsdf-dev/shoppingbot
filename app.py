...
if "flow_complete" not in st.session_state: st.session_state.flow_complete = False # 최종 안내용
...
def render_separated_ad(user_context, recommended_item_name):
    ...
    ad_res = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": f"""Recommend a DIFFERENT competitor to '{recommended_item_name}'. 
            Format: Headline | Brief Description | Price. 
            Keep it very short (max 2 sentences). Output ONLY 3 parts separated by '|'. No labels."""}
        ] + user_context
    )
    try:
...
if st.session_state.finished:
    # 광고 포함 추천 답변 생성
    is_done = any("AD" in m["content"] and "border" in m["content"] for m in st.session_state.messages)

    if not is_done:
        with st.chat_message("assistant"):
            with st.spinner("Generating recommendation..."):
                if ad_pos == "in-text":
                    sys_msg = f"Recommend ONE product. Start with a brief intro, then put {ad_tag} right before the product name. Keep it to 1 short paragraph."
                elif ad_pos == "following":
                    sys_msg = f"Recommend one product neutrally. Then, in a new paragraph, introduce a premium alternative and put {ad_tag} right before its name. Very brief."
                else:
                ...
                st.markdown(final_advice, unsafe_allow_html=True)
    ...
    if is_controllable and st.session_state.ad_pref_asked and not st.session_state.flow_complete:
        if ad_resp := st.chat_input("Type 'Keep ads' or 'Turn off ads'..."):
            ...
            if any(x in ad_resp.lower() for x in ["off", "no", "끄", "안"]):
